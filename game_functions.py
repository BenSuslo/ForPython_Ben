# 把管理事件的代码移到一个名为check_events()的函数中，
# 以简化run_game()并隔离事件管理循环。
...
def check_bullet_alien_collisons(ai_settings, screen, stats, sb, ship, aliens, bullets):
    """响应子弹和外星人的碰撞"""
    # 使用sprite.groupcollide()检测两个编组的成员之间的碰撞
    # 检查是否有子弹击中了外星人
    # 如果是这样，就删除相应的子弹和外星人
    # groupcollide()返回一个字典
    # 与外星人碰撞的子弹都是字典collisions中的一个键
    # 与每颗子弹相关的值都是一个列表，其中包含该子弹撞到的外星人
...
def get_number_rows(ai_settings, ship_height, alien_height):
    """计算屏幕可容纳多少行外星人"""
    availavle_space_y = (ai_settings.screen_height - (3 * alien_height) - ship_height)
    number_rows = int(availavle_space_y / (2 * alien_height))
    return number_rows
